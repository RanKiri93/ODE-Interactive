...
import { buildOrder3ConstantCoefficientQuestion } from "./reconstruction/order3Generation";
...
export function buildReconstructionQuestion(params: {
  ...
  if (params.equationKind === "constant-coefficients" && params.order === 3) {
    for (let attempt = 0; attempt < MAX_GENERATION_ATTEMPTS; attempt += 1) {
    ...
    return buildOrder3ConstantCoefficientQuestion({ ...params, attempt: 0 });
  }
